...
@pytest.mark.parametrize("crudo, esperado", [
    ...
    (2, 0),    # el caso del hallazgo: antes pasaba como "disco 3"
    (7, 0),    # también pasaba: era el límite viejo (< 8)
    (8, 0),
    (250, 0),
])
def test_politica_de_numero_de_disco_en_iso_plana(tmp_path, iso_bytes,
                                                  crudo, esperado):
    iso = tmp_path / "juego.iso"
    iso.write_bytes(iso_bytes(game_id=b"GZ2E01", console="gc", disc_number=crudo))
    info = read_plain_iso_header(iso)
    assert info is not None
    assert info.disc_number == esperado


@pytest.mark.parametrize("crudo, esperado", [
    (0, 0), (1, 1), (2, 0), (7, 0), (8, 0), (250, 0),
])
def test_politica_de_numero_de_disco_en_ciso(tmp_path, crudo, esperado):
    ciso = tmp_path / "juego.ciso"
    ciso.write_bytes(_ciso_bytes(disc_number=crudo))
    assert read_ciso_disc_number(ciso) == esperado


def test_un_numero_de_disco_corrupto_no_arma_un_disc3(tmp_path, iso_bytes):
